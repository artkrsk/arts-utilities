...
describe("Resize", () => {
	...
	beforeEach(() => {
		...
		// Must be a regular function expression: vitest invokes the impl via Reflect.construct
		// when callers use `new ResizeObserver(...)`, and arrow functions are not constructable.
	...
	describe("init", () => {
		...
		it("should handle environment where window is undefined", () => {
			...
			expect(consoleWarnSpy).toHaveBeenCalledWith(
				"Resize: ResizeObserver is not available.",
			);
		});
		...
		it("should handle errors from ResizeObserver constructor", () => {
			// Regular function expression so `new ResizeObserver(...)` actually invokes the throw.
