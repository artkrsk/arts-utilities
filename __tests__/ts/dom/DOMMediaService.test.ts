...
describe("DOMMediaService", () => {
	...
	describe("playVideo", () => {
		...
		it("should handle video play promise rejection gracefully", async () => {
			...
			// Should not throw
			await expect(
				DOMMediaService.playVideo(videoElement),
		...
		it("should handle null video element gracefully", async () => {
			await expect(
				DOMMediaService.playVideo(null as any),
			).resolves.toBeUndefined();
		});

		it("should handle undefined video element gracefully", async () => {
			await expect(
				DOMMediaService.playVideo(undefined as any),
			).resolves.toBeUndefined();
		});
		...
		it("should handle video element without play method", async () => {
			...
			await expect(
				DOMMediaService.playVideo(invalidVideo),
			).resolves.toBeUndefined();
		});
	...
	describe("postMessageToIframe", () => {
		...
		it("should handle null iframe element gracefully", () => {
			expect(() =>
				DOMMediaService.postMessageToIframe(null as any, "message"),
			).not.toThrow();
		});

		it("should handle undefined iframe element gracefully", () => {
			expect(() =>
				DOMMediaService.postMessageToIframe(undefined as any, "message"),
			).not.toThrow();
		});
		...
		it("should handle empty message gracefully", () => {
			expect(() =>
				DOMMediaService.postMessageToIframe(iframeElement, ""),
			).not.toThrow();
		...
		it("should handle null message gracefully", () => {
			expect(() =>
				DOMMediaService.postMessageToIframe(iframeElement, null as any),
			).not.toThrow();
			expect(mockContentWindow.postMessage).not.toHaveBeenCalled();
		});
		...
		it("should handle complex JSON messages", () => {
			...
			DOMMediaService.postMessageToIframe(
				iframeElement,
				complexMessage,
		...
		it("should work with different iframe types", () => {
			...
			const youtubeMessage = JSON.stringify({
				event: "command",
				func: "playVideo",
			});
			DOMMediaService.postMessageToIframe(
