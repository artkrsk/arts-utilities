...
import {
	detectMediaFromURL,
	getMediaType,
} from "../../../src/ts/core/media/MediaTypeDetection";
	...
	describe("detectMediaFromURL", () => {
		describe("YouTube detection", () => {
			it("should detect standard YouTube URLs", () => {
				expect(
					detectMediaFromURL("https://www.youtube.com/watch?v=dQw4w9WgXcQ"),
				).toBe("youtube");
				expect(
					detectMediaFromURL("https://youtube.com/watch?v=dQw4w9WgXcQ"),
				).toBe("youtube");
				expect(
					detectMediaFromURL("http://www.youtube.com/watch?v=dQw4w9WgXcQ"),
				).toBe("youtube");
			});

			it("should detect YouTube short URLs", () => {
				expect(detectMediaFromURL("https://youtu.be/dQw4w9WgXcQ")).toBe(
					"youtube",
				);
				expect(detectMediaFromURL("http://youtu.be/dQw4w9WgXcQ")).toBe(
					"youtube",
				);
			});
			...
			it("should detect YouTube embed URLs", () => {
				expect(
					detectMediaFromURL("https://www.youtube.com/embed/dQw4w9WgXcQ"),
				).toBe("youtube");
				expect(
					detectMediaFromURL("https://youtube.com/embed/dQw4w9WgXcQ"),
				).toBe("youtube");
			});
			...
			it("should detect YouTube mobile URLs", () => {
				...
				expect(
					detectMediaFromURL("http://m.youtube.com/watch?v=dQw4w9WgXcQ"),
				).toBe("youtube");
			});
		...
		describe("Vimeo detection", () => {
			it("should detect standard Vimeo URLs", () => {
				...
				expect(detectMediaFromURL("https://www.vimeo.com/123456789")).toBe(
					"vimeo",
				);
			});
			...
			it("should detect Vimeo player URLs", () => {
				expect(
					detectMediaFromURL("https://player.vimeo.com/video/123456789"),
				).toBe("vimeo");
				expect(
					detectMediaFromURL("http://player.vimeo.com/video/123456789"),
				).toBe("vimeo");
			});

			it("should detect Vimeo channel URLs", () => {
				expect(
					detectMediaFromURL("https://vimeo.com/channels/staffpicks/123456789"),
				).toBe("vimeo");
			});
		...
		describe("File extension fallback", () => {
			it("should fall back to getMediaType for non-platform URLs", () => {
				expect(detectMediaFromURL("https://example.com/video.mp4")).toBe(
					"video",
				);
				expect(detectMediaFromURL("https://example.com/audio.mp3")).toBe(
					"audio",
				);
				expect(detectMediaFromURL("https://example.com/image.jpg")).toBe(
					"image",
				);
			});
			...
			it("should return null for unsupported file types", () => {
				expect(
					detectMediaFromURL("https://example.com/document.pdf"),
				).toBeNull();
				expect(
					detectMediaFromURL("https://example.com/archive.zip"),
				).toBeNull();
			});
		});
