...
import {
	extractVideoID,
	generateEmbedURL,
} from "../../../src/ts/core/media/VideoURLUtils";
	...
	describe("extractVideoID", () => {
		describe("YouTube ID extraction", () => {
			...
			it("should extract ID from YouTube mobile URLs", () => {
				...
				expect(extractVideoID("http://m.youtube.com/watch?v=dQw4w9WgXcQ")).toBe(
					"dQw4w9WgXcQ",
				);
			});
			...
			it("should extract ID from YouTube URLs with additional parameters", () => {
				expect(
					extractVideoID("https://youtube.com/watch?v=dQw4w9WgXcQ&t=10s"),
				).toBe("dQw4w9WgXcQ");
				expect(extractVideoID("https://youtu.be/dQw4w9WgXcQ?t=10s")).toBe(
					"dQw4w9WgXcQ",
				);
			});
		...
		describe("Vimeo ID extraction", () => {
			it("should extract ID from standard Vimeo URLs", () => {
				...
				expect(extractVideoID("https://www.vimeo.com/123456789")).toBe(
					"123456789",
				);
			});

			it("should extract ID from Vimeo player URLs", () => {
				expect(extractVideoID("https://player.vimeo.com/video/123456789")).toBe(
					"123456789",
				);
				expect(extractVideoID("http://player.vimeo.com/video/123456789")).toBe(
					"123456789",
				);
			});
			...
			it("should extract ID from Vimeo channel URLs", () => {
				expect(
					extractVideoID("https://vimeo.com/channels/staffpicks/123456789"),
				).toBe("123456789");
			});
		...
		describe("Edge cases", () => {
			it("should return null for unsupported platforms", () => {
				...
				expect(
					extractVideoID("https://dailymotion.com/video/abc123"),
				).toBeNull();
			});
	...
	describe("generateEmbedURL", () => {
		...
		describe("Options handling", () => {
			...
			it("should handle privacy option only", () => {
				const youtubeResult = generateEmbedURL(
					"https://youtube.com/watch?v=dQw4w9WgXcQ",
					{
						privacy: true,
					},
				...
				const vimeoResult = generateEmbedURL("https://vimeo.com/123456789", {
					privacy: true,
				});
				expect(vimeoResult).toBe(
